...
class _CrochetControlScreenState extends State<CrochetControlScreen> {
  ...
  Future<void> _loadPieces() async {
    ...
    String? savedPieces = prefs.getString('pieces');
    if (savedPieces != null) {
      setState(() {
        pieces = List<Map<String, dynamic>>.from(
            jsonDecode(savedPieces).map((item) => Map<String, dynamic>.from(item)));
      });
    }
  }
  ...
  Future<void> _savePieces() async {
    SharedPreferences prefs = await SharedPreferences.getInstance();
    await prefs.setString('pieces', jsonEncode(pieces));
  }

  void _addPiece(Map<String, dynamic> pieceData) {
    setState(() {
      pieces.add({...pieceData, 'id': DateTime.now().toString()});
    });
    _savePieces();
  }
  ...
  void _updatePiece(String id, Map<String, dynamic> pieceData) {
    setState(() {
      int index = pieces.indexWhere((piece) => piece['id'] == id);
      if (index != -1) {
        pieces[index] = {...pieceData, 'id': id};
      }
    });
    _savePieces();
  }
...
class PieceCard extends StatefulWidget {
  ...
  PieceCard(
      {required this.piece, required this.onUpdate, required this.onDelete, required this.onEdit});
